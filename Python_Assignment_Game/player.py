...
class Player():
    def __init__(self):
        ...
        self.inventory = [
            items.NanoWeb(), items.WebAttack(),items.AntidoteLeaves()]
        self.hp = 100  # Hit Points
        self.maxHp = 200
        self.location_x, self.location_y = world.Webspunn
        self.victory = False 
        self.experience = 0
        self.level = 0
        self.money = 40
    ...
    def choseTransport(self):
        nmode = int(input("choose speed of WebSpuning: "))
        return items.Transport().Spiderlegs(nmode)
    ...
    def lockpass(self):
        yourpwd = input('enter the 6 digit password to start the game:')
        if(yourpwd == self.password):
            #sounds.SpaceSound()
            print('Welcome to world of Spiderman')

        else:
            print('### you have entered Mysterio password, you will be eliminated')
    ...
    def level_up(self):
        ...
        self.experience += 60
        self.level += 1
        self.inventory.append(items.Titan())
        print("Leveled up {}.** Hitpoint Increased {} and XP to {}**.Ultron is awarded to you".format(
    ...
    def equip(self):
        ...
        while True:
            itemChoice = int(input(
                """\nSelect the weapon you want to equip with: """)) - 1
